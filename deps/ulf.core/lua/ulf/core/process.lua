...
local hooks = require("ulf.core.hooks")
local timer = require("ulf.core.timer")
...
-- 	self.reading = false
-- 	self.handle = handle
-- 	self:on("pause", util.bind(self._onPause, self))
-- end
--
...
---comment
---@return ulf.GlobalProcess
local function global_process()
	---@class ulf.GlobalProcess:ulf.Emitter
	---@field argv table
	---@field exit_code integer
	---@field cwd fun():string
	---@field kill fun(pid:integer,signal:string)
	---@field next_tick fun(...:any)
	local process = Emitter:new()
	process.argv = args
	process.exit_code = 0
	process.next_tick = next_tick
	-- process.env = lenv
	-- process.cwd = cwd
	process.kill = kill
	...
	hooks:on("process.exit", util.bind(process.emit, process, "exit"))
	hooks:on("process.uncaughtException", util.bind(process.emit, process, "uncaughtException"))
	return process
end

return { global_process = global_process }
